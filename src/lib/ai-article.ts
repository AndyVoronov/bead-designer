...
export async function generateContent(topic: string, title: string, requirements?: string, products?: ProductInfo[]): Promise<string> {
  // Build product-specific additions to system and user prompts
  let productSystemAddition = "";
  let productCtaSection = "";

  if (products && products.length > 0) {
    const productList = products.map((p) => `— «${p.name}» (${p.basePrice.toLocaleString("ru-RU")} ₽${p.recommendedAge ? `, возраст ${p.recommendedAge}` : ""}${p.shortDescription ? `: ${p.shortDescription}` : ""})`).join("\n");

    productSystemAddition = `

## ПЕРСОНАЛИЗИРОВАННЫЙ CTA — ССЫЛКИ НА КОНКРЕТНЫЕ ТОВАРЫ
В статье должны быть органично упомянуты следующие товары бренда:
${productList}
Упоминай их естественно в контексте статьи — в подходящих секциях, где это релевантно. Не вставляй все упоминания в один абзац, а распредели по тексту.
CTA-блок в конце должен ссылаться на конкретные товары, а не просто на каталог. Используй персонализированный CTA-блок с карточками товаров.`;

    const productCards = products.map((p) => `    <a href="/catalog/${p.slug}" class="blog-cta-product-card">
      <img src="${p.imageUrl || "/api/uploads/products/placeholder.jpg"}" alt="${p.name}" />
      <div>
        <span class="blog-cta-product-name">${p.name}</span>
        <span class="blog-cta-product-price">${p.basePrice.toLocaleString("ru-RU")} ₽</span>
      </div>
    </a>`).join("\n");

    productCtaSection = `
### 10. CTA секция с товарами (ОБЯЗАТЕЛЬНО — с конкретными товарами)
<div class="blog-cta-section blog-reveal">
  <h2>Готовы выбрать <span class="blog-gradient-text">идеальный подарок</span>?</h2>
  <p>Описание — 2 предложения, упоминающее конкретные товары из списка и их пользу для ребёнка</p>
  <div class="blog-cta-products">
${productCards}
  </div>
</div>`;
  }
  ...
  const raw = await callLLM(
    [
      { role: "system", content: CONTENT_SYSTEM_PROMPT + productSystemAddition },
      {
        role: "user",
...
${productCtaSection || `<div class="blog-cta-section blog-reveal">
  <h2>Готовы выбрать <span class="blog-gradient-text">идеальную игрушку</span>?</h2>
  <p>Описание с акцентом на пользу для ребёнка</p>
  <a href="/catalog" class="blog-cta-button">Перейти в каталог →</a>
</div>`}

НАПОМИНАНИЕ: Это пример ФОРМАТА. Напиши свой уникальный контент 3000-5000 слов.${products && products.length > 0 ? ` В статье органично упомяни выбранные товары из списка выше — распредели упоминания по тексту, а CTA-блок в конце содержит карточки с конкретными товарами и ссылками на них.` : ""}`,
      },
    ],
    16384
  );
